...
for (index_slice in 0:slice_num){ 
  spotmarker_data = read.table(sprintf("C:/Users/wzd/Desktop/Alignment Domain Prediction/admb/select_gene/%s/select_gene_4.txt",index_slice),header = T,check.names = FALSE)
  ...
  write.table(result_gene_morans,sprintf("C:/Users/wzd/Desktop/Alignment Domain Prediction/admb/select_gene/%s/gene_morans_4.txt",index_slice),row.names = F, quote = F, sep="\t",col.names = T)
}
